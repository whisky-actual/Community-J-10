...
_dummy_pl_horiz             = CreateElement "ceSimple"
_dummy_pl_horiz.name        = "hud_ladder_hori_0"
_dummy_pl_horiz.init_pos    = {0, 0, 0}
_dummy_pl_horiz.isvisible   = false
_dummy_pl_horiz.collimated  = true
_dummy_pl_horiz.element_params = {"HUD_BRIGHTNESS", "ROLL_HUD", "PITCH_HUD","DOG_Mod_HUD"}
_dummy_pl_horiz.controllers    = {
	--{"opacity_using_parameter" ,0}, 
	{"rotate_using_parameter",1, 1 }, 
	{"move_up_down_using_parameter",2,  -0.53},
	{"hud_check_declutter"},
	{"parameter_in_range" ,3,0.9,1.1},
	}
--_dummy_pl.parent_element = fpm_name
AddElementObject(_dummy_pl_horiz)


function add_pitch_ladder(tbl, ismain, isdoghoriz)
    for k,i in pairs(tbl) do
        ishori = 0
		if isdoghoriz == 1 then
			k = 19
		end
        ...
        elseif i < 0 then
			...
			if isdoghoriz ~= 1 then
				...
				tex_poly.vertices   = {{ 289.778, 5.6505},
									   { 289.778,-5.6505},
									   {-289.778,-5.6505},
									   {-289.778, 5.6505},}
			end
		...
		if isdoghoriz ~= 1 then
			...
			tex_poly.parent_element = _dummy_pl_horiz.name
		end
...
add_pitch_ladder(ladder_main, 1, 0)
add_pitch_ladder(ladder_apr, 0, 0)
add_pitch_ladder({0}, 1, 1)
