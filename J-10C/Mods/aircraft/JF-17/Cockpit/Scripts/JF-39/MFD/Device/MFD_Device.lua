...
update_time_step = 1/24 --update will be called 24 times per second
...
local Conf_Boot_Seconds = update_time_step
local Display_Boot_Timer = 0
...
local ADI_ABS_PITCH = get_param_handle("ADI_ABS_PITCH")
...
SELECTED_PYLON = get_param_handle("SELECTED_PYLON")
...
local WARNING_TEXT = get_param_handle("WARNING_TEXT")
...
function post_initialize()
    ...
    elseif birth=="GROUND_COLD" then
		Conf_Boot_Seconds = 60
    end	
end

function BootSequence()
	if get_param_handle("Electric_Main_bus_A_115200VAC"):get() == 1 and get_param_handle("CryptoErased"):get() == 0 then 

		if Display_Boot_Timer < Conf_Boot_Seconds then
			Display_Boot_Timer = Display_Boot_Timer + update_time_step
		elseif Display_Boot_Timer >= Conf_Boot_Seconds then
			Display_Boot_Timer = Conf_Boot_Seconds
			get_param_handle("DisplaysBooted"):set(1)
		end
	else
		Display_Boot_Timer = 0
		get_param_handle("DisplaysBooted"):set(0)
	end
...
-- ADI====================================================================================================================================
	...
	ADI_ABS_PITCH:set(math.abs(sensor_data.getPitch()))
	--ADI_PITCH:set(math.pi / 4)
	--ADI_ABS_PITCH:set(math.pi / 4)
...
-- Flight data ========================================================================================================================
 ...
 -- Speed====================================================================================================================================
	...
	local curias = sensor_data.getIndicatedAirSpeed() * 1.94384449
	...
	CUR_IAS:set(sensor_data.getIndicatedAirSpeed() * 1.94384449)
	CUR_TAS:set(tSpeed)
	
	if curias <= 100 then
        SPEEDOMETER_IAS:set(curias * 0.888)
    elseif curias <= 150 then
        SPEEDOMETER_IAS:set(-90 + curias * 1.8)
	--elseif curias >= 800 then
		--SPEEDOMETER_IAS:set(-90 + 800 * 1.8)
    else
        SPEEDOMETER_IAS:set(48 + curias * 0.888)
    end



	if sensor_data.getMachNumber() >= 0.95 and sensor_data.getMachNumber() <= 1.05 then
		WARNING_TEXT:set("TRANSONIC")
	else
		WARNING_TEXT:set(" ")
	end
...
-- Engine ====================================================================================================================================
	...
	CURR_RPM = get_param_handle("RM12_RPM"):get()
