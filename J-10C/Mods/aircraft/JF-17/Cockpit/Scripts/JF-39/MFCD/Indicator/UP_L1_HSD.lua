...
local DMAP_RENDER             = CreateElement "ceTexPoly"
DMAP_RENDER.name              = "DMAP_RENDER"
DMAP_RENDER.init_pos          = {0, FWD_COMP_BIAS, 0}
DMAP_RENDER.vertices          = verts
DMAP_RENDER.tex_coords        = {{0.5 - 1/3, 0.1},
                                 {0.5 + 1/3, 0.1},
                                 {0.5 + 1/3, 1},
                                 {0.5 - 1/3, 1}}
DMAP_RENDER.indices           = DEF_BOX_INDICES
if use_ed_render_target then
    DMAP_RENDER.material      = "render_target_" .. tostring(GetRenderTarget() + 1)
else
    DMAP_RENDER.material      = "DMAP_Material_" .. tostring(GetRenderTarget()) -- 自定义材质
end
DMAP_RENDER.controllers       = {{"check_map_onoff"},}
DMAP_RENDER.level             = HSD_NAV_LEVEL - 1
DMAP_RENDER.h_clip_relation   = h_clip_relations.COMPARE
DMAP_RENDER.additive_alpha    = true
AddToUpper(DMAP_RENDER)

local DMAP_FILTER1              = CreateElement "ceMeshPoly"
DMAP_FILTER1.name               = "DMAP_FILTER1"
DMAP_FILTER1.material           = MFCD_MATERIAL_BLACK1
...
local DMAP_FILTER2              = CreateElement "ceMeshPoly"
DMAP_FILTER2.name               = "DMAP_FILTER2"
DMAP_FILTER2.material           = MFCD_MATERIAL_BLACK2
DMAP_FILTER2.primitivetype      = "triangles"
DMAP_FILTER2.init_pos           = {0, FWD_COMP_BIAS, 0}
DMAP_FILTER2.vertices           = {{-1,  1.2}, { 1,  1.2}, { 1, -1.5}, {-1, -1.5}}
DMAP_FILTER2.indices            = DEF_BOX_INDICES --{0,1,2,0,2,3 }
--DMAP_FILTER2.h_clip_relation  = h_clip_relations.REWRITE_LEVEL
DMAP_FILTER2.level              = HSD_NAV_LEVEL - 1
DMAP_FILTER2.isdraw             = true
DMAP_FILTER2.isvisible          = true
DMAP_FILTER2.use_mipfilter      = false
DMAP_FILTER2.controllers        = {{"check_map_filter", 1},}
DMAP_FILTER2.collimated         = false
DMAP_FILTER2.additive_alpha     = true
AddToUpper(DMAP_FILTER2)
...
local osb_txt = {
    ...
    {value="R\nW\nR", init_pos={MFCD_FONT_L_VERT_X, MFCD_FONT_LR2_Y}, align="LeftCenter",  ctrls={{"hsd_rwr_state_txt"}}},
	
    {value="A\nA",    init_pos={MFCD_FONT_L_VERT_X, MFCD_FONT_LR3_Y}, align="LeftCenter",  ctrls={{"hsd_contact_filter_type"}}},
    {value="M\nA\nP", init_pos={MFCD_FONT_R_VERT_X, MFCD_FONT_LR4_Y}, align="RightCenter",},
    {value="T\nC\nN", init_pos={MFCD_FONT_R_VERT_X, MFCD_FONT_LR5_Y}, align="RightCenter",},

    {value="DCLT",    init_pos={MFCD_FONT_UD5_X, MFCD_FONT_D_UY}, align="CenterCenter"},
}
...
local texs_local = {
	...
	-- D OSB box
    {-- DCLT
        cx = MFCD_FONT_UD5_X, cy = MFCD_FONT_D_UY, hw = 116/160 * 266.667/2000, hh = 133.333/2000, ulx =  72, uly = 1770, w = 116, h = 80,
        ctrls = {{"hsd_dclt_box", 0}},
    },
}
...
fwd_compass.controllers     = {{"compass_elem_rotate", 1.0},{"hsd_dclt_hidden"}}
...
fwd_compass_out.controllers     = {{"compass_elem_rotate", 1.0},{"hsd_dclt_keep"}}
...
--ac_sym.controllers     = {{"hsd_dclt_hidden"}}
ac_sym.isdraw          = true
...
hdg_sym.controllers     = {{"hsd_dclt_hidden"}}
hdg_sym.isdraw          = true
...
for i = 1,4 do
    ...
    compass_eswn.controllers    = {{"move", mx*GetScale(), my*GetScale(), 0},
                                   {"compass_elem_rotate", -1.0},{"hsd_dclt_hidden"}}
